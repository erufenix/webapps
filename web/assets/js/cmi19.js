$(document).ready(function() {
  ...
  $("#hotel").val('');
  $("#transfer").val('');
  $("#imgHotel").attr('src',asset + '/img/hotel/bell.png');
  $("#imgHotel").attr('alt','bell.png');
  $("#imgHotel").attr('title','bell.png');
  $("#image-caption").html('Select hotel');
  $(".departure").val(''); 
  ...
  $("#hotel").change(function(event) {
    ...
    if(id != 0){
      ...
      $("#imgHotel").attr('src',asset + '/img/hotel/cmi/' + img);
  ...
  $("#arrive_date").datetimepicker({
    minDate: moment('2019-09-29'),
    maxDate: moment('2019-10-02'),       
    format: 'DD/MM/YYYY',
    ignoreReadonly: true,
  ...
  $("#transfer").change(function(event) {
    var e = $(this);
    var i = $(this).val();
    var s = $("#transfer option:selected");
    var t = s.attr('data-type');
    var r = !!s.attr('data-round');
    rated_round(t,r,i);      
  });
  ...
  function rated_round(type,round=false,realtype=''){
    var rated_arrive    = 0;
    ...
    switch (type) {
      case 'shared':
        c = transferJson[realtype].value;
        v_arrive    = (c*1)*p_arrive/2;
        //v_departure = (c*1)*p_departure/2;
        v_departure = v_arrive;
        $("#note-transport").addClass('hidden').html('');
        if(round === false){
          v_arrive = (c*1)*p_arrive
          v_departure = 0;
          $("#note-transport").removeClass('hidden').html('<span class="fa fa-exclamation-circle"></span> Transport only one way mode. <b>Airport to Hotel');
        }
  ...
  if ( $("#trp").length ){
    ...
    .on('success.form.fv', function(e) {
      ...
      $("#one-mode").addClass('hidden');
      if($("#transfer").val() == 'shared_one_way' || $("#transfer").val() == 'private_one_way' ){
        $("#one-mode").removeClass('hidden');
      } 
