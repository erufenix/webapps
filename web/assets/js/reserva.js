$(document).ready(function() {
  ...
  (idHotel !== 0) ? $("#idHotel").val(idHotel) : $("#idHotel").val('');
	...
	$("#htnombre").html('<span class="has-error">'+ (lang == 'es' ? "Seleccionar el hotel" : "Selet hotel") + '</span>');
		...
		$.each(habitaciones, function(index, val) {
				if (typeof(val.sep) !== 'undefined') {
					...
					habs += '<div class="row from-group">\n'+
									'	<div class="col-sm-7 col-md-7 hbtipo"><label class="normal pointer" for="htc' + (index + 1 ) + '">' + ((val.tipo[lang] == '') ? '&nbsp' : val.tipo[lang])  + (val.hagotada == true ? ' <span class="label label-warning c-font-slim">Agotada</span>' : '') + '</label></div>\n'+
									'	<div class="col-sm-3 col-md-3 hbcosto"><label class="normal pointer" for="htc' + (index + 1 ) + '">' + currency + ' $' + ((val.costo[cur] == '') ? '&nbsp;' : val.costo[cur]  ) + '</label></div>\n'+
									'	<div class="col-sm-2 col-md-2 hbradio">\n'+
									'			<div class="c-radio">\n'+
									'				<input class="c-radio htc" name="habitacionc" id="htc' + (index + 1 ) + '" value="' + val.tipo[lang]+ "|" + val.costo[cur] + "|" + val.propinas[cur] + "|" + val.pack + '|' + val.pp +'|' + val.costor[cur] + '" type="radio" required' + (val.hagotada == true ? ' disabled' : '') + ' >&nbsp;\n'+
	...
	if(blkAco === true){
		$('.htc').on('click', function(e) {
			if(e.target.value !== undefined){
				...
				if(s.indexOf("Doble") >= 0 || s.indexOf("Triple") >= 0 || s.indexOf("Cuádruple") >= 0 || s.indexOf("DBL") >= 0){
	...
	$("#idHotel").change(function(event) {
		...
		if(value !== 0){
		  ...
		  if(all){
			...
			$('#frmRsv').formValidation('enableFieldValidators', 'pagoPor', true);
		  $('#frmRsv').formValidation('revalidateField', 'pagoPor');
			...
			$("#httumb").attr('src', protocol + host + assets + '/img/hotel/source.png');
			$("#htnombre").html('<span class="has-error">'+ (lang == 'es' ? "Seleccionar el hotel" : "Selet hotel") +'</span>');
			$("#habitaciones").html('<div class="text-center"><span class="has-error">'+(lang == 'es' ? "Seleccionar el hotel" : "Selet hotel")+'</span></div>');
			$("#msgs").html('');
		}
		valDates(all);
	});
