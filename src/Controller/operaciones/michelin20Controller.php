...
class michelin20Controller implements ControllerProviderInterface {
  private $model  =   null;

  public function connect(Application $app) {
    $index = $app['controllers_factory'];
    $index->get('/{tipo}','Controller\operaciones\michelin20Controller::index')
      ->bind('michelin20.index')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff');
    $index->get('/{tipo}/registro/','Controller\operaciones\michelin20Controller::registro')
      ->bind('michelin20.registro')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff');
    $index->get('/{tipo}/datos/','Controller\operaciones\michelin20Controller::datos')
      ->bind('michelin20.datos')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff');   
    $index->post('/valida','Controller\operaciones\michelin20Controller::valida')->bind('michelin20.valida');
    $index->post('/setregistro','Controller\operaciones\michelin20Controller::setRegistro')->bind('michelin20.setRegistro');
    $index->post('/setdatos','Controller\operaciones\michelin20Controller::setDatos')->bind('michelin20.setDatos');
    $index->get('/{tipo}/sede','Controller\operaciones\michelin20Controller::sede')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.sede');
    $index->get('/{tipo}/agenda/','Controller\operaciones\michelin20Controller::agenda')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.agenda');
    $index->get('/{tipo}/contacto/','Controller\operaciones\michelin20Controller::contacto')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.contacto');
    $index->get('/{tipo}/vuelos/','Controller\operaciones\michelin20Controller::vuelos')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.vuelos');
    $index->get('/{tipo}/pago/','Controller\operaciones\michelin20Controller::pago')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.pago');

    $index->get('/{tipo}/reporte/','Controller\operaciones\michelin20Controller::reporteMi')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.reporteMi');
      
    $index->get('/{tipo}/reporteT/','Controller\operaciones\michelin20Controller::reporteMiTaller')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.reporteMiT');

    $index->get('/{tipo}/reporteTaco/','Controller\operaciones\michelin20Controller::reporteMiTallerAco')
      ->assert('tipo', '\w+')
      ->value('tipo', 'staff')
      ->bind('michelin20.reporteMiTaco');      

    $index->post('/tvalida','Controller\operaciones\michelin20Controller::tvalida')->bind('michelin20.tvalida');
    $index->post('/sendContacto','Controller\operaciones\michelin20Controller::sendContacto')->bind('michelin20.sendContacto');
    $index->get('/reporte','Controller\operaciones\michelin20Controller::reporte')->bind('michelin20.reporte');
    $index->get('/reporte2','Controller\operaciones\michelin20Controller::reporte2')->bind('michelin20.reporte2');
    $index->get('/checkout','Controller\operaciones\michelin20Controller::checkout')->bind('michelin20.checkout');
    $index->get('/execute','Controller\operaciones\michelin20Controller::execute')->bind('michelin20.execute');
    $index->post('/pagoCompleto','Controller\operaciones\michelin20Controller::payComplete')->bind('michelin20.payComplete');
    $index->get('/payCancel','Controller\operaciones\michelin20Controller::payCancel')->bind('michelin20.payCancel');
    return $index;
  }

  public function index(Request $request, Application $app,$tipo) {
    $page = "index.twig";
    return $app['twig']->render("pages/michelin20/$page", array(
          'title' => 'inicio',
          'tipo'  => $tipo
    ));
  }
  ...
  public function setRegistro(Request $request, Application $app) {
    ...
    if(!empty($data)){
      ...
      $mail
        ...
        ->setSubject('Michelin Simposio Alliance 2020');
        $body = $app['twig']->render('pages/michelin20/rmail.twig', array(
            "data"      => $data,
  ...
  public function datos(Request $request, Application $app,$tipo){
    $page = $tipo. "_datos.twig";
    return $app['twig']->render("pages/michelin20/$page", array(
          'title'     => ($tipo == 'staff') ? 'Datos staff' : 'Datos distribuidores',
          'tipo'      => $tipo,
          'talleresM' => $this->getTalleres($app,'m'),
          'talleresT' => $this->getTalleres($app,'t')
    ));
  ...
  public function setDatos(Request $request, Application $app) {
    $model  =   $app["michelin20Model"];
    $tsm    = array();
    $stm    = $request->request->get('stm');
    $tst    = array();
    ...
    if(!empty($request->request->get('tt'))){
      foreach ($request->request->get('tt') as $km => $vm) {
        $ar     = explode("|", $stt[$km]);
        $tst[]  = $vm ." - " . $ar[1];
        $tb[$km+3]['noTaller'] = $ar[2];
        $tb[$km+3]['idTaller'] = $ar[0];
        $tb[$km+3]['hrTaller'] = $ar[1];
        $tb[$km+3]['tTaller']  = 't';
        $tb[$km+3]['taller']   = $vm;
        $id     = $ar[0];
        $up     = $model->upTaller($id,$staff);
    ...
    if(!empty($request->request->get('atm'))){
      foreach ($request->request->get('atm') as $km => $vm) {
        $ar     = explode("|", $astm[$km]);
        $atsm[]  = $vm ." - " . $ar[1];
        $tab[$km]['noTaller'] = $ar[2];
        $tab[$km]['idTaller'] = $ar[0];
        $tab[$km]['hrTaller'] = $ar[1];
        $tab[$km]['tTaller']  = 'm';
        $tab[$km]['taller']   = $vm;        
        $id     = $ar[0];
        $up     = $model->upTaller($id,$staff);
    ...
    if(!empty($request->request->get('att'))){
      foreach ($request->request->get('att') as $km => $vm) {
        $ar     = explode("|", $astt[$km]);
        $atst[]  = $vm ." - " . $ar[1];
        $tab[$km+3]['noTaller'] = $ar[2];
        $tab[$km+3]['idTaller'] = $ar[0];
        $tab[$km+3]['hrTaller'] = $ar[1];
        $tab[$km+3]['tTaller']  = 'm';
        $tab[$km+3]['taller']   = $vm;
        $id     = $ar[0];
        $up     = $model->upTaller($id,$staff);
    ...
    $atalleresT = implode("\n",$atst);
    $request->request->set('talleres_aco',$atalleresM . "\n\n" . $atalleresT);
    $reg = $model->uTaller($tb,$tab,$request->request->get('idr'));
    return true;
  }

  public function sede(Request $request, Application $app,$tipo) {
    return $app['twig']->render('pages/michelin20/sede.twig', array(
          'title' => 'sede',
          'tipo'  => $tipo
    ));
  }
  ...
  public function contacto(Request $request, Application $app,$tipo) {
    return $app['twig']->render('pages/michelin20/contacto.twig', array(
      'title' => 'contacto',
      'tipo'  => $tipo
    ));
  }
  ...
  public function vuelos(Request $request, Application $app,$tipo) {
    return $app['twig']->render('pages/michelin20/vuelos.twig', array(
  ...
  public function pago(Request $request, Application $app,$tipo) {
    return $app['twig']->render('pages/michelin20/pago.twig', array(
      'title' => 'Pago',
      'tipo'  => $tipo
    ));
  }
  ...
  public function reporteMiTaller(Request $request, Application $app,$tipo) {
    ...
    $filename = 'Simposio_Alliance_2020_talleres';
  ...
  public function reporteMiTallerAco(Request $request, Application $app,$tipo) {
    ...
    $filename = 'Simposio_Alliance_2020_talleres_acom';
    $exporter_source = new \Exporter\Source\ArraySourceIterator($_atll);
    header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
    header('Content-Description: File Transfer');
    header('Content-type: ' . $content_type);
  ...
  public function reporteMi(Request $request, Application $app,$tipo) {
    ...
    $filename = 'Simposio_Alliance_2020';
  ...
  public function sendContacto(Request $request, Application $app) {
    ...
    $mail
      ...
      ->setSubject('Contacto Michelin Simposio Alliance 2020');
    $body = $app['twig']->render('pages/michelin20/mailContact.twig', array(
  ...
  header('Content-Disposition: attachment; filename=michelin20regs.csv' );
  ...
  public function reporte2(Request $request, Application $app) {
    ...
    $fn   = new Functions;
    $excel  = $fn->genExcel('michelin2020','michelin2020','Registros');
    $ltrnum   = 65;
    $ltrchr   = "";
    $lastchr  = "";
    $lastcell = 0;
    ...
    $excel->getActiveSheet()->setCellValue('A1', 'Michelin 2020');
    ...
    header('Content-Disposition: attachment;filename="michelin20.xlsx"');
